  ...
  const handleSubmit = async (isDraft: boolean = false) => {
    ...
    try {
      ...
      if (!isDraft) {
        ...
        // UPSC-specific validation: stricter requirements for quality content
        if (articleType === 'upsc') {
          if (contentLength < 2000) {
            toast({ 
              title: 'UPSC Content Too Short', 
              description: `UPSC articles require minimum 2,000 characters. Current: ${contentLength}`, 
              variant: 'destructive' 
            });
            return;
          }
          if (metaDescLength < 120) {
            toast({ 
              title: 'UPSC SEO Error', 
              description: `Meta description must be 120-160 characters. Current: ${metaDescLength}`, 
              variant: 'destructive' 
            });
            return;
          }
          if (excerptLength < 100) {
            toast({ 
              title: 'UPSC Excerpt Required', 
              description: `Excerpt must be at least 100 characters. Current: ${excerptLength}`, 
              variant: 'destructive' 
            });
            return;
          }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="lg:col-span-3 space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {/* UPSC Content Requirements Warning */}
              {articleType === 'upsc' && (
                <Alert variant={
                  (() => {
                    const contentLength = formData.content?.replace(/<[^>]*>/g, '').length || 0;
                    const metaDescLength = formData.meta_description?.length || 0;
                    const excerptLength = formData.excerpt?.length || 0;
                    const hasIssues = contentLength < 2000 || metaDescLength < 120 || excerptLength < 100;
                    return hasIssues ? 'destructive' : 'default';
                  })()
                } className="mb-4">
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription>
                    <strong>UPSC Article Requirements:</strong>
                    <ul className="mt-2 space-y-1 text-xs">
                      <li className={formData.content?.replace(/<[^>]*>/g, '').length >= 2000 ? 'text-green-600' : ''}>
                        Content: {formData.content?.replace(/<[^>]*>/g, '').length || 0}/2,000 chars minimum
                        {formData.content?.replace(/<[^>]*>/g, '').length >= 2000 ? ' ✓' : ' ✗'}
                      </li>
                      <li className={(formData.meta_description?.length || 0) >= 120 && (formData.meta_description?.length || 0) <= 160 ? 'text-green-600' : ''}>
                        Meta Description: {formData.meta_description?.length || 0}/120-160 chars
                        {(formData.meta_description?.length || 0) >= 120 && (formData.meta_description?.length || 0) <= 160 ? ' ✓' : ' ✗'}
                      </li>
                      <li className={(formData.excerpt?.length || 0) >= 100 ? 'text-green-600' : ''}>
                        Excerpt: {formData.excerpt?.length || 0}/100 chars minimum
                        {(formData.excerpt?.length || 0) >= 100 ? ' ✓' : ' ✗'}
                      </li>
                    </ul>
                  </AlertDescription>
                </Alert>
              ...
              <div>
                ...
                <Input
                  ...
                  className="mt-1"
                />
                <p className="text-xs text-muted-foreground mt-1">{formData.meta_title?.length || 0}/60 characters</p>
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  ...
                  {articleType === 'upsc' && (formData.meta_description?.length || 0) < 120 && (
                    <span className="text-destructive ml-1">(UPSC min: 120)</span>
                  )}
                </p>
              </div>
            </CardContent>
