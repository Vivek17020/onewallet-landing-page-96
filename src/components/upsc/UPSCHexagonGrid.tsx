    ...
    <section className="py-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
        {subjects.map((subject) => (
          ...
          >
            <div className="relative bg-white border-2 border-slate-200 rounded-2xl p-5 transition-all duration-300 hover:border-transparent hover:shadow-xl hover:-translate-y-1 overflow-hidden">
              ...
              <div className="relative z-10">
                {/* GS Paper badge */}
                <span className={`inline-block text-xs font-semibold px-2 py-0.5 rounded-full mb-3 bg-gradient-to-r ${gsColors[subject.gsPaper] || gsColors["CA"]} text-white group-hover:bg-white/20`}>
                  {subject.gsPaper}
                </span>
                
                {/* Icon */}
                <div 
                  className="w-12 h-12 rounded-xl flex items-center justify-center mb-3 transition-colors duration-300 group-hover:bg-white/20"
              ...
              {/* Decorative corner */}
              <div className="absolute -bottom-2 -right-2 w-16 h-16 rounded-full opacity-10 group-hover:opacity-20 transition-opacity" style={{ backgroundColor: subject.color }} />
            </div>
          </Link>
        ))}
      </div>
